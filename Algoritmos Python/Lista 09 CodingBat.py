...
def middle_way(a, b):
  ...
  if len(a) % 2 != 0 and len(b) % 2 != 0:
    meioA = (len(a) / 2)
    meioA = math.floor(meioA)
    meioB = (len(a) / 2)
    meioB = math.floor(meioB)
  listaNova = []
  valor_do_meioA = a[meioA]
  listaNova.append(valor_do_meioA)
  valor_do_meioB = b[meioB]
  listaNova.append(valor_do_meioB)
  return listaNova
...
def squirrel_play(temp, is_summer):
  if temp >= 60 and temp <= 90:
    return True
  elif temp < 60 or temp > 90 and is_summer == False:
    return False
  elif temp > 90 and temp <= 100 and is_summer == True:
    return True
  else:
    return False
# I. pego_correndo
